...
def readRATE (file, nums=[], nrate = 0):
    ...
    for n in range(0,nrate): 
        ...
        for j in range(0,mw):                                      
            ...
            ResArr[nrate*V*j + nrate* Sopt + n] = farr[5]   # записываем накопленную нефть дл€ добывающих
            ResArr[nrate*V*j + nrate* Swpt + n] = farr[7]   # записываем накопленную воду  дл€ добывающих
            if(wtype[j] == 1): 
                ResArr[nrate*V*j + nrate* Swpr + n] = farr[2]   # записываем дебит воды дл€ добывающих
            elif(wtype[j]==-1):
                ResArr[nrate*V*j + nrate* Swpr + n] = farr[1]   # приемистость дл€ нагнетательных  
                ResArr[nrate*V*j + nrate* Swit + n] = farr[11]   # записываем накопленную закачку дл€ нагнетательных 
        ...
        for j in range(0,mw):
            ...
            if(qwirh[j] >= 0 and wtype[j] == -1 ): ResArr[nrate*V*j + nrate*Hwpr + n] = farr[qwirh[j]]  # ¬Ќ»ћјЌ»≈, если есть приемистость, то переписываем ее вместо дебита воды!!!!  как-то сомнительно.... но на тестовой модели работает, а на реальной нет....                                                
