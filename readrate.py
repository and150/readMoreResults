...
def readRATE (file, nums=[], nrate = 0):
    ...
    # local class of 1-st time step header    
        def __init__(self, mnem="", units="", asname="", descr="", ASIND = [], ASDEPTH = []):    
            ...
            # 1 Ц Associated x-index
            # 2 Ц Associated y-index
            # 3 Ц Associated z-index
            # 4 Ц Associated local grid
            # 5 Ц Associated component index
            # 6 Ц Associated fluid in place region
    ...
    #массивы дл€ хранени€ индексов исторических показателей                
    qoprh = [-1]*mw
    qwprh = [-1]*mw
    qbhph = [-1]*mw  
    qwirh = [-1]*mw      
    ...
    for n in range(0,nrate): 
        ...
        # чтение записи дл€ одного временного шага
        ...
        for j in range(0,mw):                                      
            ...
            wtype[j] = iarr[7]  #wtype = buffArr[7]          # запоминаем текущий тип скважины (1 - добывающа€, -1 - нагнетательна€)
            ...
            ResArr[nrate*V*j + nrate* cts.Sopt + n] = farr[5]   # записываем накопленную нефть дл€ добывающих
            ResArr[nrate*V*j + nrate* cts.Swpt + n] = farr[7]   # записываем накопленную воду  дл€ добывающих
            if(wtype[j] == 1): 
                ResArr[nrate*V*j + nrate* cts.Swpr + n] = farr[2]   # записываем дебит воды дл€ добывающих
            elif(wtype[j]==-1):
                ResArr[nrate*V*j + nrate* cts.Swir + n] = farr[1]   # приемистость дл€ нагнетательных  
                ResArr[nrate*V*j + nrate* cts.Swit + n] = farr[11]   # записываем накопленную закачку дл€ нагнетательных 
            ...
            if IWVCMP == 1:                       
                ...
                farr.frombytes(line[s:f])
                for kk in range(0,mwzone): # итераци€ по сло€м модели
                   if compArr[kk] !=0:
                      ResArr[nrate*V*j + nrate*(Vbase + compArr[kk]-1) + n] = farr[kk]  # записываем дебит нефти по перфораци€м                   
                      ResArr[nrate*V*j + nrate*(Vbase + mwzone + compArr[kk] - 1) + n] = farr[kk + 2*mwzone] # записываем дебит воды(приемистость) по перфораци€м
        ...
        for j in range(0,mw):
            ...
            if(qwirh[j] >= 0 and wtype[j] == -1 ): ResArr[nrate*V*j + nrate*cts.Hwir + n] = farr[qwirh[j]]  # ¬Ќ»ћјЌ»≈, если есть приемистость, то переписываем ее вместо дебита воды!!!!  как-то сомнительно.... но на тестовой модели работает, а на реальной нет....                                                
        ...
        # возвращаем массив прочиатнных данных, повременна€ запись конвертирована в массив векторов
    #return Items
