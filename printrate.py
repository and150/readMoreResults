...
def printRate(times, numsArray, RateOut):
    ...
    for x in wellNames:
        ...
        for i in range(0,T):
                print(str(times[i].tos) +" " + str(wellNames[wi]+" "),end = ' ') # вывод времени и номера скважины                              
                ...
                print(str(ResArr[T*V*wi + T*cts.Hopt + i]), end = ' ')  # вывод фактической накопленной добычи нефти
                print(str(ResArr[T*V*wi + T*cts.Swit + i]), end = ' ')  # вывод расчетной накопленной закачки 
                print(str(ResArr[T*V*wi + T*cts.Hwit + i]), end = ' ')  # вывод фактической накопленной закачки 
                print(str(ResArr[T*V*wi + T*cts.Swpt + i]), end = ' ')  # вывод расчетной накопленной добычи воды 
                print(str(ResArr[T*V*wi + T*cts.Hwpt + i]), end = ' ')  # вывод фактической накопленной добычи воды 
