...
def get_wells_cells(out_arrays, wells_dates_filename, well_names, times_perfs, times, start_date_array, out_file, perfs_array_linear):  # gets values only for perforated cells (perfs from rate-file)
    ...
    wells_dates_from_file_1 = [[ well_names.index(*list(filter(lambda y: y.strip()==x.split()[0], well_names))), 
                                x.split()[0],
    ...
    #print(wells_dates_from_file_1) # [well_model_index, well_name, date, timestep_index] # debug
    ...
    selected_wells_perfs_1 = list([well_date[1], 
                                 well_date[2], 
                                 [well_date[3], well_date[0],
                                 *list( zip(*[iter(perfs_array_linear[ n_wells*n_vec*n_k*well_date[3] + n_vec*n_k*well_date[0] :  n_wells*n_vec*n_k*well_date[3] + n_vec*n_k*well_date[0] +n_vec*n_k])]*n_k) )]] 
                                 for well_date in wells_dates_from_file_1)
    #for item in selected_wells_perfs_1: print(item) # debug


    ''' print connection values'''
    for perf_item in selected_wells_perfs_1:
