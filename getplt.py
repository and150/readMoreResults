...
#                   print(ResArr[T*V*j + T*(6+MZ+k) + i], end=" ")     # вода
                ...
                for k in range(0,MZ):
                    ...
                    pltarr[k] = ResArr[T*V*j + T*(V+k) + i] + ResArr[T*V*j + T*(V+MZ+k) + i] # генераци€ временного вектора PLT на каждый временной шаг дл€ каждой скважины
                #вывод на консоль
                #print(pltarr)
                ...
                #    pltOutFile.write( '{} {} {:.3f} {:.3f} {:.3f}\n'.format(times[i], wellNames[j], sum(pltarr[0:4])/pltLR, sum(pltarr[4:7])/pltLR, sum(pltarr[7:10])/pltLR) )                    
                   if wellNames[j] == 'WQ2-197':
                       pltOutFile.write( '{} {} {:.3f} {:.3f} - - - \n'.format( times[i], wellNames[j],  sum(pltarr[0:46])/pltLR, sum(pltarr[46:89])/pltLR )  )                  
                       ...
                       pltOutFile.write( '{} {} {:.3f} {:.3f} {:.3f} - - \n'.format( times[i], wellNames[j],  sum(pltarr[0:40])/pltLR, sum(pltarr[40:66])/pltLR, sum(pltarr[66:89])/pltLR )  )                  
                   elif  wellNames[j]== 'WQ2-267':
                       pltOutFile.write( '{} {} {:.3f} {:.3f} {:.3f} - - \n'.format( times[i], wellNames[j],  sum(pltarr[0:44])/pltLR, sum(pltarr[45:53])/pltLR, sum(pltarr[54:89])/pltLR )  )                  
                   elif  wellNames[j]== 'WQ2-189':
                       pltOutFile.write( '{} {} {:.3f} {:.3f} {:.3f} {:.3f} - \n'.format( times[i], wellNames[j],  sum(pltarr[0:39])/pltLR, sum(pltarr[39:54])/pltLR, sum(pltarr[54:75])/pltLR , sum(pltarr[75:89])/pltLR)  )                  
                   elif  wellNames[j]== 'WQ2-20':
                       pltOutFile.write( '{} {} {:.3f} {:.3f} {:.3f} {:.3f} - \n'.format( times[i], wellNames[j],  sum(pltarr[0:32])/pltLR, sum(pltarr[32:61])/pltLR, sum(pltarr[61:73])/pltLR , sum(pltarr[73:89])/pltLR)  )                  
                   else:                	
